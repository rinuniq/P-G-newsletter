...
df_top_opportunities=identify_top_opportunities(df_final, tangible_threshold, growing_threshold)
# brand_owners=df_top_opportunities['BRAND OWNER'].unique().tolist()
df_value_drivers=preprocessing_value_drivers(file_path, sheet2)

df_merged=merging_data(df_top_opportunities, df_value_drivers)

df_priority=calculate_value_driver_contributions(df_merged)

df_risks=identify_risks(df_top_opportunities,brand_owner)
