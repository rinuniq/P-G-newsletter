...
#     """
#     <style>
#       /* ---- Production cleanup: hide Streamlit chrome ---- */
#       header[data-testid="stHeader"] { display: none; }   /* hides top header incl. Deploy menu */
#       #MainMenu { visibility: hidden; }                   /* hides hamburger menu */
#       footer { visibility: hidden; }                      /* hides "Made with Streamlit" footer */

#       /* Optional: tighten top spacing since header is gone */
#       .block-container {
...
#         logo_candidates = [
#             "NIQ-logo.png",
#             ".streamlit/NIQ-logo-bright-blue-web.png",
#             "NIQ.png",
#         ]
...
# ------------------------------------------------------------------------------------
# Global CSS (production cleanup) + NIQ header + centered hero (match the other app)
# ------------------------------------------------------------------------------------
PRIMARY = "#7B4DFF"
PRIMARY_DARK = "#5E34E6"
TEXT_MUTED = "#5B5B6A"
...
<style>
  ...
  /* ====== Clean NIQ Header (separate block; logo + CS AI CoE on one row) ====== */
  .niq-header {{
    display: flex;
    align-items: center;
    gap: 16px;
  ...
  .niq-coe {{
    font-weight: 800;
    font-size: 22px;
    color: #111111;             /* bold black look */
    line-height: 1;
    margin: 0;
  }}

  /* ====== Centered hero (big title like the PDF app) ====== */
  .hero {{
    text-align: center;
    margin-top: 6px;
    margin-bottom: 10px;
  ...
  .hero-title {{
    font-size: 44px;            /* bold center title */
    font-weight: 800;
    margin: 0;
...
# ------------------------------------------------------------------------------------
# Helper: image -> data URI (to place logo via HTML, ensuring perfect alignment)
# ------------------------------------------------------------------------------------
def img_to_data_uri_if_present(p: str | Path) -> str | None:
    p = Path(p)
    if not p.exists():
        return None
    b = p.read_bytes()
...
# ------------------------------------------------------------------------------------
# NIQ Header (separate block): logo + CS AI CoE (aligned in one row)
# ------------------------------------------------------------------------------------
logo_candidates = [
    ".streamlit/NIQ-logo-bright-blue-web.png",
    "NIQ-logo.png",
...
logo_data_uri = img_to_data_uri_if_present(logo_path) if logo_path else None

st.markdown(
    f"""
<div class="niq-header">
  {"<img class='niq-logo' src='" + logo_data_uri + "' alt='NIQ logo' />" if logo_data_uri else "<b class='niq-coe'>NIQ</b>"}
  <h1 class="niq-coe">CS AI CoE</h1>
</div>
""",
    unsafe_allow_html=True,
)
...
# ------------------------------------------------------------------------------------
# Centered hero: Title + subtitle (exactly like the other app)
# ------------------------------------------------------------------------------------
st.markdown(
    """
<div class="hero">
  <div class="hero-title">Growth Opportunities &amp; Risks Analysis</div>
  <div class="hero-subtitle">
    Upload your raw Excel workbook and get instant insights on priority brands and risk assessment.
  </div>
</div>
""",
    unsafe_allow_html=True,
...
# ------------------------------------------------------------------------------------
# File upload & configuration (unchanged logic)
# ------------------------------------------------------------------------------------
uploaded_file = st.file_uploader("Choose Excel file", type=["xls", "xlsx"])
...
# Brand owner selection (sticky)
# -----------------------------
# Set default only once; handle common encodings of ampersand
if (st.session_state.brand_owner is None) or (st.session_state.brand_owner not in unique_brand_owners):
    preferred = "PROCTER & GAMBLE"
    alt1 = "PROCTER &amp; GAMBLE"
    alt2 = "PROCTER &amp;amp; GAMBLE"
    if preferred in unique_brand_owners:
        st.session_state.brand_owner = preferred
    elif alt1 in unique_brand_owners:
        st.session_state.brand_owner = alt1
    elif alt2 in unique_brand_owners:
        st.session_state.brand_owner = alt2
    else:
        st.session_state.brand_owner = unique_brand_owners[0]
